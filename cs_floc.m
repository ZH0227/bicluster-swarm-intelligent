function biClustResult = cs_floc(k,D )
...
%                   the genes   size: nbRows, numBiclust
%   blocColumn  -   a matrix indicating the directed initialisation for
%                   the conditions  size: nbCols, numBiclust
%
% Outputs:
...
elseif D == 5
  ...
  lamda = 1;
  miu = 1000;
  [bics,~,~,~]=csb(nPop,D,lamda,miu);         %size(bics) = numBiclust*(nbRows+nbCols)
  blocRow = bics(:,1:nbRows)';                %size(blocRow) = nbRows*numBiclust
  blocColumn = bics(:,nbRows+1:size(bics,2))';%size(blocColumn) = nbCols*numBiclust
